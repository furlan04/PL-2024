...
(defun urilib-parse (uri)
  (
   ...
   (if (isSpecial (coerce (car (extract-scheme (coerce uri 'list))) 'string))
       (special-scheme (coerce (car (extract-scheme (coerce uri 'list))) 
                               'string) 
                       (cdr (extract-scheme (coerce uri 'list)))
                       )
     (let*
         (
          (scheme  (coerce (car (extract-scheme (coerce uri 'list))) 
                           'string))       
          ...
          (fragment (if (string= (first (cdr query)) "#") 
                        (coerce (extract-fragment (rest (cdr query))) 
                                'string) 
                      NIL
                      )
                    )        
...
;;; Returns a cons with result and after
...
(defun check-host (chars)
  (if (digit-char-p (first chars))
      ...
      (if (or (null (extract-host chars T)) (null (car (extract-host chars T))))
        ...
        (cons (check-ip (car (extract-host chars T))) 
              (cdr (extract-host chars T)))
        )
  ...
  (cond ((string= scheme "mailto")
         ...
         (let* ((userInfo (extract-userinfo after))
                (host  (if (contains-char after "@")
                           (coerce (car (check-host  (cdr userInfo))) 
                                   'string)))
