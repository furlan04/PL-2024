  ...
  (
   ...
   (let
       (
        ...
        (authority
         (cond
          ((isSpecial (coerce (extract-scheme (coerce uri 'list)) 'string)) 
           NIL)
      ...
      (
       ...
       :path (if (null after) NIL
               (if (string= (first after) "/")
                   (if (string= scheme "zos")
                       (if (string= (first after) "/") 
                           (coerce (zos-extract-path (rest after)) 'string)
                         (coerce (zos-extract-path after) ))
                     (if (string= (first after) "/") 
                         (coerce (extract-path (rest after)) 'string)
                       (coerce (extract-path after) 'string)))
                 (error "Missing '/'")))
       :query (if (string= (first after) "?") 
                  (coerce (extract-query (rest after)) 'string) NIL)
       :fragment (if (string= (first after) "#") 
                     (coerce (extract-query (rest after)) 'string) NIL)
       :userInfo (cond
       ...
       :host (if (not (equal authority nil))
                 (if (string= (first after) "@")
                   ...
                   (check-host after))              
               nil)
       :port (if (string= (first after) ":") 
                 (coerce (extract-port (rest after)) 'string) (get-port scheme NIL))
       )
      )
     )
...
(defun check-host (chars)
  (if (digit-char-p (first chars))
      ...
      (if (equal (extract-host chars T) nil)
          (error "no host specified")
          (check-ip (extract-host chars T)))
      ;Else it's considered a normal host
      (if (equal (extract-host chars) nil)
          (error "no host specified")
          (coerce (extract-host chars) 'string))))

;;returns the IP address coerced to a string if it is valid, 
  ...
  (
   ...
   ;; If no characters remain, return NIL
    ...
    ((or (string= (first chars) "?") (string= (first chars) "#")) 
     (defparameter after chars)
     (if (and (not (contains-at-most-one chars "?")) 
              (not (contains-at-most-one chars "#"))) 
...
;;; Ensure that the character list contains 
;;; at most one occurrence of a character
